...
import { ScatterChart as ScatterChartIconLucide, Download } from "lucide-react";
...
const DynamicScatterGraph: React.FC<ModernScatterGraphProps> = React.memo(
  ({
    ...
    // Use shared hooks
    // For scatter, groupBy acts as X-axis key, and valueKey as Y-axis key
    const {
      graphData,
      isValidGraphData,
      autoGroupBy,
    ...
    useEffect(() => {
      ...
      if (!valueKey && autoValueKey) {
        setValueKey(autoValueKey);
      }
    }, [groupBy, valueKey, autoGroupBy, autoValueKey, setGroupBy, setValueKey]);
