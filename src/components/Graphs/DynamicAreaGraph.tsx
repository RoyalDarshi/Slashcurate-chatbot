  ...
  ({
    ...
    function transformDynamicData(
      ...
      if (selectedAggregate === "count") {
        ...
        return {
          data: uniqueGroupValues.map((group) => ({
            [groupByToUse]: group,
        ...
        const groupedSums: { [key: string]: any } = {};
        rawData.forEach((row) => {
          const group = row[groupByToUse];
          ...
          if (!groupedSums[group]) {
            groupedSums[group] = { [groupByToUse]: group };
            yKeys.forEach((key) => (groupedSums[group][key] = 0));
          }
          groupedSums[group][valueKeyToUse] =
            (groupedSums[group][valueKeyToUse] || 0) +
            (isNaN(value) ? 0 : value);
        });

        const finalData = uniqueGroupValues.map((group) => {
          if (groupedSums[group]) {
            return groupedSums[group];
          } else {
            const emptyGroup: any = { [groupByToUse]: group };
            yKeys.forEach((key) => (emptyGroup[key] = 0));
            return emptyGroup;
          }
        });

        const keysForSum = valueKeyToUse ? [valueKeyToUse] : [];
  ...
  .map((entry: any, index: number) => (
              <div
                ...
                style={{
                  ...
                  background: `${entry.color || entry.stroke}15`,
                  border: `1px solid ${entry.color || entry.stroke}30`,
