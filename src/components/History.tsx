...
import { Search, Heart } from "lucide-react"; // Changed Star to Heart
...
const History: React.FC = ({onSessionClicked}) => {
  ...
  const loadSessions = () => {
    try {
      ...
      if (storedMessages) {
        ...
        const groupedSessions = groupMessagesIntoSessions(allMessages);
        setSessions(groupedSessions);
        filterSessions(groupedSessions, activeFilter);
      }
    } catch (error) {
  ...
  const groupMessagesIntoSessions = (messages: Message[]): Session[] => {
    const sessions: Session[] = [];
    let currentSession: Message[] = [];

    messages.forEach((message, index) => {
      if (!message.isBot && (currentSession.length === 0 || index === 0)) {
        if (currentSession.length > 0) {
          sessions.push({
            id: `session-${Date.now()}-${sessions.length}`,
            messages: [...currentSession],
            timestamp: currentSession[0].timestamp,
            title: currentSession[0].content.substring(0, 50) + "...",
            isFavorite: false,
          });
        }
